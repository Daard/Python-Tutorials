...
def entropy(class_probabilites):
    ...
    return sum(-p * math.log(p, 2) for p in class_probabilites
               if p) # ignore zero probabilites
...
def partition_entropy_by(inputs, attribute):
    ...
    partions = partition_by(inputs, attribute)
    return partition_entropy(partions.values())
...
def build_tree_id3(inputs, split_candidates=None):
    ...
    subtrees = {attribute_vlaue : build_tree_id3(subset, new_candidates)
                for attribute_vlaue, subset in partitions.items()}
